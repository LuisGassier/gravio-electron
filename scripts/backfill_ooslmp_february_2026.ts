/**
 * Backfill Script: Generate historical virtual records for OOSLMP (Feb 1-28, 2026)
 *
 * This script:
 * 1. Deletes all existing virtual records for February 2026
 * 2. Generates new synthetic records with realistic business rules
 *
 * Usage:
 *   DRY_RUN mode (default, no changes):
 *     SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npx ts-node scripts/backfill_ooslmp_february_2026.ts
 *
 *   LIVE mode (actually deletes and inserts):
 *     DRY_RUN=false SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... npx ts-node scripts/backfill_ooslmp_february_2026.ts
 */
...
const TARGET_TOTAL_KG = 2540200 // 2,540.20 tons for February 2026 (including physical records)
const COLLISION_BUFFER_MINUTES = 25 // Minimum minutes between ANY truck entries (relaxed from 15 to fit less trips/day over less days)
const VEHICLE_COOLDOWN_MINUTES = 360 // 6 hours - minimum time before same truck return
const FORCED_WORKING_DAYS = new Set(['2026-02-15', '2026-02-22'])

// Date range: February 1-28, 2026 (México time UTC-6)
// Note: Database stores in UTC, so we need to adjust
const START_DATE = new Date(Date.UTC(2026, 1, 1, 6, 0, 0)) // Feb 1 00:00 Mexico (UTC-6) -> 06:00 UTC
const END_DATE = new Date(Date.UTC(2026, 2, 1, 5, 59, 59)) // Mar 1 00:00 Mexico = Feb 28 23:59:59 Mexico -> Mar 1 06:00 UTC (-1 sec)
...
async function getWorkingDays(): Promise<string[]> {
  ...
  for (const dayKey of daysToCheck) {
    if (FORCED_WORKING_DAYS.has(dayKey)) {
      workingDays.push(dayKey)
      console.log(`   ✅ Día laborable forzado por regla de negocio: ${dayKey}`)
      continue
    }
